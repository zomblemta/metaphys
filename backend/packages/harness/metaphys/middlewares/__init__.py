...
1. :class:`ToolErrorHandlingMiddleware` —— 兜底未预期异常；必须在最外层才能拦住
   其它中间件抛出的异常
2. :class:`SafetyMiddleware` —— 高风险输入与输出的有限句式规则
3. :class:`GroundingMiddleware` —— 核验模型输出里的命盘数据是否都来自工具
4. :class:`ClarificationMiddleware` —— 追问时结束本轮、并丢弃同轮兄弟工具调用；
   **必须永远最后**
"""
...
from metaphys.middlewares.safety import SafetyMiddleware
