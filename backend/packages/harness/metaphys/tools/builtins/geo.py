...
但这个判据有个前提：用户给的是**短名**，需要靠我们去猜他指哪一处。若他直接写了
完整行政区划名（「北京市朝阳区」），地方已经指死了，此时判歧义不只是多余 —— 它是
个**死循环**：短名「朝阳」跨三个城市，模型照例列出候选请用户选，而用户无论照抄
候选名还是报编号，都无法表达"就是北京市朝阳区"，因为判据只看短名。所以全名必须
先于短名命中（``lookup_candidates`` 的 ``by_full``）。
